...
FOLDER_NAME='Phonon-Bandstructure'
...
#%%
if not os.path.exists(FOLDER_NAME):os.mkdir(FOLDER_NAME) and os.chdir(FOLDER_NAME)

print ("===================================================================")
print (" Starting Band Calculation                                         ")
os.system(GNUBANDSBIN+" -e "+ str(int(float(minE[0]))-1)+" -E "+str(int(float(maxE[0]))+1)+' < ' + gnuband_input+'  > '+'./'+FOLDER_NAME+'/bandoutput') 
...
plt.ylim(float(efermi[0])+float(minE[0]),float(efermi[0])+float(maxE[0]))
...
plt.savefig('phonon-bandstructure.pdf')
plt.close()
